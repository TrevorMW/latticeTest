...
class Learndash_Memberpress_Tools
{
	...
	public function retroactive_access_process()
	{
		...
		$per_batch   = apply_filters( 'learndash_memberpress_retroactive_tool_per_batch', 100 );
		...
		if ( $percentage >= 100 ) {
			...
			// Transactions
			$query = "SELECT id, user_id, product_id, trans_num, status, created_at, expires_at, subscription_id FROM {$mepr_db->transactions} LIMIT {$per_batch} OFFSET {$offset}";
